...
check_user_mail <- function(x) {
  ...
  check2 <- stringi::stri_detect(regex = "edu\\..+$|\\.edu$|ac\\..+$|\\.ac$|ugr\\..+$|\\.ugr$|UBC.CA|\\.cn|unav.es$|yakebiotech.com$|unige.ch$|.nl$|gov|tec.mx$|petermac.org|carrerasresearch.org|vladimirmascience|1650464505", 
                                 str=as.character(x))
  check1 & check2
}
